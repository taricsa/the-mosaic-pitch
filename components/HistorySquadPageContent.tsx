...
export default function HistorySquadPageContent({ squad }: { squad: Squad }) {
  const { currentLanguage } = useLanguage();
  const t = getDictionary(currentLanguage).historySquadPage;
  const visual = SQUAD_VISUAL_CONFIG[squad];
  const copy = t.squads[squad];

  const campaigns = useMemo(
    () => [...visual.campaigns].sort((a, b) => b.year - a.year),
    [visual.campaigns],
  );
    ...
    <div className="flex min-h-full flex-col bg-zinc-950 text-zinc-50">
      <header className="relative overflow-hidden border-b border-zinc-800/80">
        ...
        <div className="relative mx-auto max-w-6xl px-6 py-16 sm:px-10 sm:py-20 lg:px-16">
          ...
          >
            <span aria-hidden>{visual.emblem}</span>
            {copy.label}
          </p>
          ...
          >
            {(["men", "women"] as const).map((key) => (
              ...
              >
                {key === "men" ? t.mensHistoryNav : t.womensHistoryNav}
              </Link>
            ))}
          </nav>
        </div>
